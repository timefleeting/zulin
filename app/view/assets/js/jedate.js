...
;(function(window, factory) {
    ...
    jeDate.parse = jet.parse = function(ymdhms, format) {
        return format.replace(new RegExp(regymdzz,"g"), function(str, index) {
        	if(str=='DD'){ //bug: 选择月不选日期出错undefined
        			ymdhms[str] = typeof ymdhms[str]=='undefined' ? 1 : ymdhms[str];
        	}
    ...
    function jeDatePick(elem, options){
        var config = {
            ...
            currShow:{YYYY:false,MM:false,DD:false}, //显示当前日期面板
            format:"YYYY-MM-DD hh:mm:ss",               //日期格式
            minDate:"1900-01-01 00:00:00",              //最小日期
            maxDate:"2099-12-31 23:59:59",              //最大日期
            isShow:true,                                //是否显示为固定日历，默认值 isShow: true，为true时是点击显示，为false的时候固定显示
            ...
            //参数一 是一个数组，参数二是一个布尔值 如果参数二为false，则设置你要的默认值
    ...
    jet.extend(jeDatePick.prototype,{
        init : function () {  
            ...
            var getCurrValue = function () {
                ...
                if( jet.isObj(curVal[0]) ){  
                		if(!isEmpty){  //isEmpty==false 无操作值  isEmpty==true有操作值
                			opts.currShow.YYYY ? curVal[0].YYYY=opts.currShow.YYYY : '';
                			opts.currShow.MM   ? curVal[0].MM=opts.currShow.MM   : '';
                			opts.currShow.DD   ? curVal[0].DD=opts.currShow.DD   : '';
                		}
                		//bug修复:显示选中的值,当点击确定的时候要默认为选中的值
                		that.selectValue = [curVal[0].YYYY+'-'+jet.digit(curVal[0].MM)+'-'+jet.digit(curVal[0].DD)];	
                }
                /*注意还有个curVal[1] nextMonth下个月 */
            ...
            if(!isShow || !trigges){ 
                ...
                if (trigges) {
                    jet.on(that.valCell,trigges,function(){ 
                        ...
                        that.minDate = jet.isType(opts.minDate,"function") ? opts.minDate(that) : opts.minDate;
                        that.maxDate = jet.isType(opts.maxDate,"function") ? opts.maxDate(that) : opts.maxDate;
                        var gvarr = getCurrValue(); //修改:置后以便minDate或maxDate可以更新初始化参数
                       	//console.log(opts);
        ...
        /*
         * curr {YYYY:int,MM:int,DD:int,hh:int,mm:int,ss:int,zz:int}
         */
        ...
        dateTemplate : function() {
            ...
            '{% for(var y=0;y<=11;y++){ %}<td class="{%=yearlist[i][y].style%}" @on="yearClick({%=yearlist[i][y].y%})"><span>{%=yearlist[i][y].y%}'+ytxt+'</span></td>{% if((y+1)%3==0){ %} </tr>{% } %} {% } %}<tr><td @on="yearPrevPage({%=yearlist[i][0].y%})"><span>上一页</span></td><td></td><td @on="yearNextPage({%=yearlist[i][0].y%})"><span>下一页</span></td></tr> </tbody></table>';
            //循环月的模板
        ...
        methodEventBind : function() {
            ...
            jet.extend(that,{
                ...
                yearPrevPage:function(val){
                		var num = parseInt(val)-11;
                		that.yearBtn('lprev',num.toString());
                },
                yearNextPage:function(val){
                		var num = parseInt(val)+11;
                		that.yearBtn('rnext',num.toString());
                },
