...
class Image
{
    ...
    public function water($source, $water, $savename = null, $alpha = 80)
    {
        ...
        //生成混合图像
        imagecopymerge($sImage, $wImage, $posX, $posY, 0, 0, $wInfo['width'], $wInfo['height'],
            $alpha);
    ...
    function showImg($imgFile, $text = '', $x = '10', $y = '10', $alpha = '50')
    {
        ...
        $info = $this->getImageInfo($imgFile);
        if ($info !== false)
        {
            ...
            if ($im)
            {
                ...
                if (!empty($text))
                {
                    ...
                    { //判断$text是否是图片路径
                        // 取得水印信息
                        $textInfo = $this->getImageInfo($text);
                        ...
                        imagecopymerge($im, $waterMark, $x, $y, 0, 0, $textInfo['width'], $textInfo['height'],
                            $alpha);
                    } else
                    {
     ...
     * @return void
     */
    function thumb($image, $thumbname, $type = '', $maxWidth = 200, $maxHeight =
    50, $interlace = true)
    {
        // 获取原图信息
        ...
        if ($info !== false)
        {
            ...
            } elseif ('gif' == $type)
            {
                ...
                if ($trnprt_indx >= 0)
                {
                    ...
                    $trnprt_indx = imagecolorallocate($thumbImg, $trnprt_color['red'], $trnprt_color['green'],
                        $trnprt_color['blue']);
     ...
     * @return void
     */
    function thumb2($image, $thumbname, $type = '', $maxWidth = 200, $maxHeight =
    50, $interlace = true)
    {
        // 获取原图信息
        $info = $this->getImageInfo($image);
        if ($info !== false)
        {
            ...
            // 复制图片
            if (function_exists("ImageCopyResampled"))
                imagecopyresampled($thumbImg, $srcImg, 0, 0, $srcX, $srcY, $maxWidth, $maxHeight,
                    $cutWidth, $cutHeight);
            else
                imagecopyresized($thumbImg, $srcImg, 0, 0, $srcX, $srcY, $maxWidth, $maxHeight,
                    $cutWidth, $cutHeight);
     ...
     * @return string
     */
    function buildString($string, $rgb = array(), $filename = '', $type =
    'png', $disturb = 1, $border = true)
    {
        if (is_string($size))     //?
            $size = explode(',', $size);
        $width = $size[0];
        ...
        $pointColor = imagecolorallocate($im, mt_rand(0, 255), mt_rand(0, 255), mt_rand
        (0, 255)); //点颜色
        ...
        if (!empty($disturb))
        {
            ...
            } elseif ($disturb = 2 || $disturb = 3)
            {
                for ($i = 0; $i < 10; $i++)
                {
                    imagearc($im, mt_rand(-10, $width), mt_rand(-10, $height), mt_rand(30, 300),
                        mt_rand(20, 200), 55, 44, $pointColor);
                }
            }
        }
     ...
     * @return string
     */
    function buildImageVerify($length = 4, $mode = 1, $type = 'png', $width =
    48, $height = 22, $verifyName = 'verifyImg')
    {
        require_once('String.class.php');
        $string_obj = string::instance();
        $randval = $string_obj ->randString($length, $mode);
        session($verifyName, md5($randval));              //保存session
        ...
        {
            imagearc($im, mt_rand(-10, $width), mt_rand(-10, $height), mt_rand(30, 300),
                mt_rand(20, 200), 55, 44, $stringColor);
        }
        for ($i = 0; $i < 25; $i++)
        {
    ...
    // 中文验证码
    function GBVerify($length = 4, $type = 'png', $width = 180, $height = 50,
                             $fontface = 'simhei.ttf', $verifyName = 'verify')
    {
        require_once('String.class.php');
        $string_obj = string::instance();
        $code = $string_obj->randString($length, 4);
        $width = ($length * 45) > $width ? $length * 45 : $width;
        session($verifyName, md5($code));               //保存session
        $im = imagecreatetruecolor($width, $height);
        $borderColor = imagecolorallocate($im, 100, 100, 100); //边框色
        ...
        for ($i = 0; $i < 15; $i++)
        {
            $fontcolor = imagecolorallocate($im, mt_rand(0, 255), mt_rand(0, 255), mt_rand(0,
                255));
            imagearc($im, mt_rand(-10, $width), mt_rand(-10, $height), mt_rand(30, 300),
                mt_rand(20, 200), 55, 44, $fontcolor);
        }
        for ($i = 0; $i < 255; $i++)
        {
            $fontcolor = imagecolorallocate($im, mt_rand(0, 255), mt_rand(0, 255), mt_rand(0,
        ...
        for ($i = 0; $i < $length; $i++)
        {
            $fontcolor = imagecolorallocate($im, mt_rand(0, 120), mt_rand(0, 120), mt_rand(0,
                120)); //这样保证随机出来的颜色较深。
            $codex = $string_obj->msubstr($code, $i, 1);
            imagettftext($im, mt_rand(16, 20), mt_rand(-60, 60), 40 * $i + 20, mt_rand(30,
                35), $fontcolor, $fontface, $codex);
        }
        $this->output($im, $type);
    }
     ...
     * @return string
     */
    function showASCIIImg($image, $string = '', $type = '')
    {
        $info = $this->getImageInfo($image);
        if ($info !== false)
        {
            ...
            for ($y = 0; $y < $dy; $y++)
            {
                for ($x = 0; $x < $dx; $x++)
                {
                    ...
                    $out .= sprintf('<span style="margin:0px;color:#%02x%02x%02x">' . $str .
    ...
    function UPCA($code, $type = 'png', $lw = 2, $hi = 100)
    {
        ...
        for ($x = 0; $x < strlen($bars); $x++)
        {
            ...
            ImageFilledRectangle($im, ($x * $lw) + 15, 5, ($x + 1) * $lw + 14, $hi + 5 + $sh,
                $color);
        }
